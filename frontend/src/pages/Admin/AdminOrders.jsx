...
export default function AdminOrders() {
      ...
      if (response.data.success) {
        ...
        if (response.data.counts) {
          setCounts({
            ...
            refunded: response.data.counts.refunded || 0
          });
        }
  ...
  const statusTabs = [
    ...
    { key: 'refunded', label: 'Refunded', icon: <RollbackOutlined />, color: 'text-purple-500', bg: 'bg-purple-50' },
  ];
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      ...
      case 'refunded':
        return <span className="bg-purple-100 text-purple-700 px-3 py-1 rounded-full text-[10px] font-bold uppercase"><RollbackOutlined className="mr-1" /> Refunded</span>;
      default:
